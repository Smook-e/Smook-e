...
📦 **[ProductTracker](https://github.com/Smook-e/ProductTracker)** — a product-monitoring backend in FastAPI, PostgreSQL, Redis, and Celery. Scraping runs as async background jobs via Celery + Redis instead of blocking API requests, with Celery Beat handling daily price checks automatically. Added Redis caching on the read path, which took throughput from ~53 to ~1280 req/s under load testing. Built with a Factory Pattern so new e-commerce sites can be plugged in without touching the core.
